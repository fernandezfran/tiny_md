...
void forces(const double *rxyz, double *fxyz, double *epot, double *pres,
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j += 2) {
            ...
            //                si x >  L/2 => x -= L
            mask =
                _mm_cmplt_pd(rx, mcell_side2); // devuelve rx si sí, si no 0 (?)
            mask = _mm_and_pd(
                rx, mask); // 1 si true 0 si false       (?)
            ...
            __m128d rij2 =
                _mm_add_pd(_mm_add_pd(_mm_mul_pd(rx, rx), _mm_mul_pd(ry, ry)),
                           _mm_mul_pd(rz, rz));
            ...
            if (rij2 <= rcut2) { // no sé como hacer este if
                ...
                __m128d fr = _mm_mul_pd(
                    a24, _mm_mul_pd(_mm_sub_pd(_mm_mul_pd(a2, r6inv), a1),
                ...
                pot = _mm_add_pd(pot,
                                 _mm_sub_pd(_mm_mul_pd(_mm_mul_pd(a4, r6inv),
                                                       _mm_sub_pd(r6inv, a1)),
                                            ect));
                // *pres += fr*rij2;
