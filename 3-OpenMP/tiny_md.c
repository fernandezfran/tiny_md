...
void init_vel(double *vxyz, double *temp, double *ekin){
    ...
    for (int j = 0; j < 3; j++) sumv[j] /= (double)N; 
        ...
        reduction(+:pot,pres_vir)
    {
        ...
        for (int i = from; i < to; i++){
            ...
            for (int j = 0; j < N; j++){
                ...
                for (int k = 0; k < 3; k++) rij[k] = pbc(rij[k], L);
